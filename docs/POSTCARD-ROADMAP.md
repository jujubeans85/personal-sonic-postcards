...
## Portable project identity and cross-device masters
The browser shelf is a local working cache, never the cross-device authority. Recommended master location: iCloud Drive / CRATE JUICE / POSTCARDS / PROJECTS. The user explicitly chooses to put exported files there; no background photo upload or iCloud integration is implied. Preserve both existing iPhone/iPad files, copy them to that folder under distinct names, verify reopening, and only then decide what to archive. We have not accessed or moved those device files.

Saved documents now carry stable project identity, unique revision ID, parent revision, human name and timestamps. Saves are immutable new versions; equal revision numbers on two devices do not imply equal content or authority. A separate-postcard action starts a new identity. Legacy imports are supported and deterministically identified by validated content; different legacy content is never merged merely because names match. Pick the intended file explicitly before editing, one device at a time. This is safe portable versioning, not an automatic conflict-resolving sync engine.

## Coordination across chats
GitHub main is the source of truth for the app; individual chat claims are not release evidence. PR #6 is a separate draft for help/layout and deployment diagnosis. The portable-versioning change owns project saving and identity; do not duplicate either implementation in another chat. Check latest main and open PRs before editing shared maker files. Merge one tested change at a time, reconcile markup conflicts explicitly, then verify the deployed version. Preserve the versioning controls, import logic and iCloud/local-working-copy explanation when moving help text. The five Ps used here are Proper Planning Prevents Poor Performance.
