...
export const listProjects=async()=>(await run('readonly',s=>s.getAll())).map(({id,name,updated,document})=>({id,name,updated,document})).sort((a,b)=>b.updated-a.updated);
export const getProject=id=>run('readonly',s=>s.get(id));
export const removeProject=id=>run('readwrite',s=>s.delete(id));
export async function saveProject({id,name,blob,document}){id=id||crypto.randomUUID();await run('readwrite',s=>s.add({id,name:name.slice(0,100),blob,document,updated:Date.now()}));return id;}
