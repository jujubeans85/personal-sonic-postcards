...
import {loadHandwriting} from '../../shared/handwriting.mjs?v=titlefit1';
...
import {preparedPhoto,renderCard,drawQR} from '../../shared/postcard-renderer.mjs?v=titlefit1';
...
const history=[];let thumbKey='',paintFrame=0,lastPreviewSide=null,previewNeedsText=false;
...
function paint(){
 valid=false;previewNeedsText=false;say('');$('qr-options').hidden=!project.qrEnabled;$('preview').style.background=project.stock==='recycled'?'#e4d6b6':'#fff';$('strength-value').textContent=project.intensity+'%';$('reset-style').setAttribute('aria-pressed',String(project.style==='original'));$('intensity').disabled=project.style==='original';
 try{
  ...
  const previewErrors=[];renderCard(staged,project,photoFor(),side,qr,0,previewErrors);previewNeedsText=previewErrors.length>0;Object.assign($('preview'),d);$('preview').getContext('2d').drawImage(staged,0,0);lastPreviewSide=side;$('text-fit').textContent=`${project.title.length}/100 title · ${project.message.length}/800 message. `+staged.textFits.join(' · ');$('preview-state').textContent=(side==='front'?'Front':'Back')+' preview · '+(side==='back'?(project.postal?'Postal layout on':'Plain back')+' · ':'')+(project.qrEnabled?(qr?'QR ready on '+project.qrSide:'QR needs a URL'):'QR off');
  if(previewErrors.length)throw Error(previewErrors.join(' '));
 ...
 }catch(e){say(e.message);$('preview-state').textContent=lastPreviewSide?(previewNeedsText?`${lastPreviewSide==='back'?'Back':'Front'} draft: text needs attention. Layout and QR shown; exports paused.`:`Showing rendered ${lastPreviewSide}. Check the message below; exports paused.`):'Preview needs attention. Check the message below.';}
 for(const side of ['front','back'])$('view-'+side).setAttribute('aria-pressed',String($('view').value===side));
 $('print').textContent=project.sides==='both'?'Print card · front + back':'Print '+project.sides;for(const id of ['png','share','pdf','print'])$(id).disabled=!valid||busy;
}
...
for(const [k,v] of Object.entries(project)){const el=$(k);if(!el||['title','recipient','message','signature','address','returnAddress','qrURL'].includes(k))continue;el.addEventListener(el.type==='range'?'input':'change',()=>{if(el.type!=='range')remember();project[k]=el.type==='checkbox'?el.checked:typeof v==='number'?Number(el.value):el.value;if(k==='qrEnabled'&&project.qrEnabled){project.qrSide='back';project.sides='both';$('view').value='back';}if(k==='postal'&&project.postal)applyPostal();if(k==='qrSide'&&project.qrEnabled){$('view').value=project.qrSide;project.sides='both';}sync();if(el.type==='range')schedulePaint();else paint();});}
...
function applyPostal(){Object.assign(project,{postal:true,width:150,height:105,photoSide:'front',sides:'both',qrSide:'back'});$('view').value='back';}
$('postal-format').onclick=()=>{remember();applyPostal();sync();paint();};
$('show-qr').onclick=()=>{$('view').value=project.qrSide;paint();};
loadHandwriting().then(paint).catch(e=>say(e.message));
$('format').onchange=()=>{if($('format').value==='custom')return;remember();[project.width,project.height]=$('format').value.split(',').map(Number);project.postal=$('format').value==='150,105';if(project.postal){project.photoSide='front';project.sides='both';$('view').value='back';}sync();paint();};
$('view').onchange=paint;
...
$('collection-link').onclick=()=>{remember();const u=new URL('../',location.href);const slug=new URL(location.href).searchParams.get('t');project.qrURL=slug?shareURL(u.href,slug):u.href;project.qrEnabled=true;project.qrSide='back';project.sides='both';$('view').value='back';sync();paint();};
...
$('remove-project').onclick=async()=>{const id=$('saved-projects').value;if(!id)return;try{await removeProject(id);if(shelfId===id)shelfId=null;await refreshShelf();$('shelf-status').textContent='Removed from this device. Downloaded backups are unchanged.';}catch(e){$('shelf-status').textContent=e.message;}};
refreshShelf();
