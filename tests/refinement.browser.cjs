...
try{
 ...
 await page.locator('#restore-index').setInputFiles(backup);await page.waitForFunction(()=>document.querySelector('#index-status').textContent.includes('merged'));assert.match(await page.locator('#index-status').innerText(),/merged/);index=await page.evaluate(()=>JSON.parse(localStorage.getItem('juice-link-index-v1')));assert.equal(index.links.length,1);
