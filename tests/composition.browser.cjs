...
(async()=>{
 ...
 await page.addInitScript(()=>{Storage.prototype.setItem=()=>{throw Error('Persistence is forbidden');};indexedDB.open=()=>{throw Error('Persistence is forbidden');};});
 ...
 const saved=await download('save','postcard-test.juicecard'),savedData=JSON.parse(await fs.readFile(saved,'utf8'));assert.equal(savedData.project.style,'deco');assert.deepEqual(Buffer.from(savedData.photo.split(',')[1],'base64'),await fs.readFile(photo));
