...
(async()=>{
 ...
 const base=process.env.POSTCARD_TEST_URL||'http://127.0.0.1:8765';if(process.env.TEST_BUNDLE){await page.goto(base+'/START_HERE.html');assert.equal(await page.locator('a').first().getAttribute('href'),'collections/maker/');}await page.goto(base+'/collections/maker/?t=vintage-3');
 ...
 await page.evaluate(()=>{const result=[{transcript:'wrong target'}];result.isFinal=true;testRecognition.onresult({resultIndex:0,results:[result]});});
 assert.equal(await page.locator('#title').inputValue(),'MY WORDS');
 ...
 const exportedPixels=await page.evaluate(async({project,photo})=>{
  ...
  const source=new Image();source.src=photo;await source.decode();const qr=qrcode(0,'M');qr.addData(qrDestination(project.qrURL),'Byte');qr.make();
 ...
 },savedData);assert.deepEqual(Buffer.from(exportedPixels,'base64'),await fs.readFile(png));
 ...
 // A new microphone session started while an import is awaiting File.text must also be cancelled.
 await page.evaluate(()=>{const read=File.prototype.text;File.prototype.text=function(){File.prototype.text=read;return new Promise(resolve=>{globalThis.finishImport=()=>read.call(this).then(resolve);});};});
 await page.locator('#open').setInputFiles(saved);await page.waitForFunction(()=>typeof finishImport==='function');
 await page.locator('#speak-recipient').click();await page.evaluate(()=>finishImport());
 await page.waitForFunction(()=>document.querySelector('#speak-recipient').getAttribute('aria-pressed')==='false');
 await page.evaluate(()=>{const result=[{transcript:'late after reopen'}];result.isFinal=true;testRecognition.onresult({resultIndex:0,results:[result]});});
 assert.equal(await page.locator('#recipient').inputValue(),'For Mimi');
 ...
 await page.locator('#title').fill('LOVE 🦄');assert.match(await page.locator('#status').innerText(),/no capture/);assert.equal(await page.locator('#png').isDisabled(),true);
 await page.locator('#font').selectOption('preset');assert.equal(await page.locator('#png').isDisabled(),false);
 const unsupported=await browser.newPage();await unsupported.addInitScript(()=>{globalThis.SpeechRecognition=undefined;globalThis.webkitSpeechRecognition=undefined;});
 await unsupported.goto(base+'/collections/maker/');await unsupported.locator('#speak-title').click();assert.match(await unsupported.locator('#speak-title + p').innerText(),/keyboard/);await unsupported.close();
 assert.deepEqual(external,[]);assert.deepEqual(errors,[]);console.log('PASS: both captured fonts, glyph bounds, missing characters, unsupported speech, cross-field/import cancellation, exact PNG renderer parity and reopened preview, 15 presets, real photo, shared rendering, undo, QR, PNG, PDF, original-byte project roundtrip, invalid import/replacement, no persistence or external requests, mobile overflow, reload clearing, gallery routes.');
 await browser.close();
